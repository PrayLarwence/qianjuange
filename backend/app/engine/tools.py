...
规则：
1. 你看到的世界状态包括：世界全貌大纲（如有）、**角色人格速查**、实体清单、近期事件、未解决的因果钩子、当前 tick；如果世界有地图，还会包含地图概览与各实体当前坐标。
...
4. 你必须通过调用工具来改变世界 —— 不要只用文字描述事件，要用 add_event、update_entity 等工具落到结构化数据里。
5. 每一步至少推进 1 个 tick（advance_time），并产生 1-3 个有意义的事件。事件之间尽量用 link_causality 建立因果，但**不要为了"看起来连贯"硬连不相关事件**——只在确实有因果时才连。
6. 引入新实体时填写 summary 和 attributes，不要留空。如果是 character，填一份起码包含 drives 与 voice 的 persona——这能让后续推演保持一致。
7. 当世界存在地图时：
